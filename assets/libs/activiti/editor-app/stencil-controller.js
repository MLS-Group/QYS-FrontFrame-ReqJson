...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.showMeTheDetails = function (index,property) {
            ...
            jQuery('body').append("<div style='position: absolute;right: 21.3%;top:"+top+"px;width: 300px;height: 80px;background: #ffffff;font-size: 16px;padding: 16px 0 0 16px;border: 1px solid #d2d2d2;border-radius: 2px;' id='mydiv2'>"+property.title+":&nbsp;&nbsp;"+pvalue+"<div class='triangle-right' style='position: absolute;right: -2.53333333%;top: 50%;'></div>"+"<div class='triangle-right-small' style='position: absolute;right: -2.13333333%;top: 50.5%;'></div>"+"</div>")
        };
        ...
        $scope.updatePropertyInModel = function (property, shapeId) {
            ...
            for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
                if ($scope.selectedItem.properties[i].key=="oryx-initiator") {
                    $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                    $scope.selectedItem.properties[i].value = "initiator";
                }
...
//                }
            ...
            if (property.key&&property.key=="oryx-formtypeselect") {
                ...
                if (temporary == "externalForm") {
                    for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
                        console.log($scope.selectedItem.properties);
                        if ($scope.selectedItem.properties[i].key=="oryx-formkeydefinition") {
                            console.log("Shiny Days!");
                            $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/string-property-write-mode-template.html";
                            $scope.selectedItem.properties[i].readModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                            //$scope.propertyClicked(i);
                        }
                    }
                } else if (temporary == "oldForm"||temporary =="newForm") {
                    for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
                        if ($scope.selectedItem.properties[i].key=="oryx-formkeydefinition") {
                            $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/form-keydefinition-select-template.html";
                            $scope.selectedItem.properties[i].readModeTemplateUrl = "editor-app/configuration/properties/form-keydefinition-display-template.html";
                            //$scope.propertyClicked(i);
                        }
                    }
                }
...
//            if (property.key&&property.key=="oryx-usertaskgroup") {
//                if (property.value) {
//                    //console.log(property.value);
//                    var str = '[{"value":"'+property.value+'","$$hashKey":"'+property.$$hashKey+'"}]';
//                    var assignment = '{"assignment": {"candidateGroups": [{"value":"' + property.value + '","$$hashKey":"' + property.$$hashKey + '"}]}}';
//                    }
//                    
//                    for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
//                        console.log($scope.selectedItem.properties[i].key);
//                        if ($scope.selectedItem.properties[i].key=="oryx-usertaskassignment") {
//                            
...
////                                $scope.selectedItem.properties[i].value.assignment = {};
////                                $scope.selectedItem.properties[i].value.assignment.candidateGroups = [];    
////                                var candidateGroup = {"value":property.value, "$$hashKey":property.$$hashKey};
////                                $scope.selectedItem.properties[i].value.assignment.candidateGroups.push(candidateGroup);
//                            }
...
//            }
            ...
            if (property.value&&property.value=="Parallel") {
                console.log($scope.selectedItem.properties);
                for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
                    if ($scope.selectedItem.properties[i].key=="oryx-multiinstance_cardinality") {
                        //$scope.selectedItem.properties[i].value = "assigneeList";
                        $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.propertyClicked(i);
                    }
                    if ($scope.selectedItem.properties[i].key=="oryx-multiinstance_collection") {
                        $scope.selectedItem.properties[i].value = "assigneeList";
                        $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.propertyClicked(i);
                    }
                    if ($scope.selectedItem.properties[i].key=="oryx-multiinstance_variable") {
                        $scope.selectedItem.properties[i].value = "assignee";
                        $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.propertyClicked(i);
                    }
                    if ($scope.selectedItem.properties[i].key=="oryx-multiinstance_condition") {
                        if (!$scope.selectedItem.properties[i].value) {
                            $scope.selectedItem.properties[i].value = '${actMultiInsCompleteService.complete(execution,"all")}';
                        }
                        $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/multiinstance-property-pass-template.html";
                        $scope.propertyClicked(i);
                    }
                    if ($scope.selectedItem.properties[i].key=="oryx-usertaskassignment") {
                        $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html?";
                        ...
                        if (typeof $scope.selectedItem.properties[i].value != "string"&&$scope.selectedItem.properties[i].value) {
            ...
            if (property.value&&property.value=="None") {
                ...
                for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
                    ...
                    if ($scope.selectedItem.properties[i].key=="oryx-usertaskassignment") {
                        if ($scope.selectedItem.properties[i].value&&$scope.selectedItem.properties[i].value.assignment.assignee == "${assignee}") {
                            //$scope.selectedItem.properties[i].value = "";
                            delete $scope.selectedItem.properties[i].value.assignment.assignee;
                            //$scope.selectedItem.properties[i].value.assignment.assignee = "${assignee}";
                        }
                        $scope.selectedItem.properties[i].writeModeTemplateUrl = "editor-app/configuration/properties/assignment-select-template.html";
                        //$scope.propertyClicked(i);
                    }
                }
            }
            ...
            for (var i = 0;i<$scope.selectedItem.properties.length;i++) {
                //console.log($scope.selectedItem.properties[i]);
                if ($scope.selectedItem.properties[i].key=="oryx-setvalueinitiator") {
                    if ($scope.selectedItem.properties[i].value==true) {
                        $scope.selectedItem.properties[i+1].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        //console.log($scope.selectedItem.properties[i+1].value.assignment.assignee,$scope.selectedItem.properties[i+1].value.assignment.assignee,$scope.selectedItem.properties[i+1].value.assignment.assignee);
                        if (!$scope.selectedItem.properties[i+1].value) {
                            $scope.propertyClicked(i+1);
                            $scope.selectedItem.properties[i+1].value = {assignment:{assignee:"${initiator}"}};
                        } else {
                            $scope.selectedItem.properties[i+1].value.assignment.assignee = "${initiator}";
                        }
                        $scope.selectedItem.properties[i-2].value = "";
                        $scope.selectedItem.properties[i-2].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.selectedItem.properties[i-3].value = "";
                        $scope.selectedItem.properties[i-3].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.selectedItem.properties[i-4].value = "";
                        $scope.selectedItem.properties[i-4].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.selectedItem.properties[i-5].value = "";
                        $scope.selectedItem.properties[i-5].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.selectedItem.properties[i-6].value = "None";
                        $scope.selectedItem.properties[i-6].writeModeTemplateUrl = "editor-app/configuration/properties/default-value-display-template.html";
                        $scope.propertyClicked(i-6);
                        //$scope.selectedItem.properties[i-6].value = "";
                    } else if($scope.selectedItem.properties[i].value==false&&$scope.selectedItem.properties[i+1].value.assignment.assignee == "${initiator}") {
                        delete $scope.selectedItem.properties[i+1].value.assignment.assignee;
                        $scope.selectedItem.properties[i+1].writeModeTemplateUrl = "editor-app/configuration/properties/assignment-select-template.html";
                        $scope.selectedItem.properties[i-6].writeModeTemplateUrl = "editor-app/configuration/properties/multiinstance-property-write-template.html";
                        //$scope.propertyClicked(i);
                    }
